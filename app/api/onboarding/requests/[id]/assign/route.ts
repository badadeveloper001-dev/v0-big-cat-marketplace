...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (existing.assigned_agent_id && existing.assigned_agent_id !== agent_id) {
      return NextResponse.json({ success: false, error: 'Already assigned to another agent' }, { status: 409 })
    }
